#!/usr/bin/env python3
"""
ltx_director_onefile.py

BASELINE LOCK: this file captures the known-good Yui/LTX Director baseline.
Do not change graph topology, baked workflow structure, model defaults, resolution
path, sampler/step defaults, guide strength, stage-2 denoise, or other quality-
sensitive values unless José explicitly approves that specific experiment first.
Prompt text, segment args, keyframe paths, output names, and CLI overrides are the
intended safe customization surface.
...
DEFAULT_LORA = "ltx-2.3-22b-distilled-lora-1.1_fro90_ceil72_condsafe.safetensors"
...
# Recovered known-good Yui/LTX quality setting from ComfyUI job
# d908ed3a-bf0a-424a-b9cc-7aac70993c9b. Keep this locked unless José
# explicitly approves changing the recipe.
DEFAULT_STAGE2_DENOISE = 0.42
...
def main() -> None:
    ...
    ap.add_argument("--lora", default=DEFAULT_LORA, help="Patch LoraLoaderModelOnly.lora_name. Defaults to DEFAULT_LORA at top of file.")
    ...
    ap.add_argument("--stage2-denoise", type=float, default=DEFAULT_STAGE2_DENOISE, help="Patch second-stage BasicScheduler.denoise. Default is recovered known-good Yui value 0.42.")
